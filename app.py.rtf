...
st.title("\uc0\u55357 \u56550  3D Packing Simulator")\
\
# --- SIDEBAR: INPUTS ---\
...
\
    ...
    if len(b_result.unfitted_items) > 0:\
        st.error("\uc0\u10060  These items were too big to fit:")\
        for item in b_result.unfitted_items:\
            st.write(f"- \{item.name\}")\
\
else:\
    st.info("\uc0\u55357 \u56392  Change settings in the sidebar and click 'Pack Items'")}